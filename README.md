...
- [x] **普通优先级** — 复现成功
- [x] **部分环** — 复现成功
- [x] **放置顺序带来的影响** — 已完全实现。Graph 的 Kahn BFS 分层拓扑排序中，同一层内的节点按 placement index（即 JSON `components` 数组中的出现顺序）排序。这一设计使得放置顺序直接影响 topo_index 和同层组件的处理顺序，进而影响 Splitter/ProtocolStash 的距离分组和 round-robin 行为。测试用例 `A.json` 与 `B.json` 验证了 Converger + Splitter 在不同下游拓扑下的分流行为差异。
- [ ] **环的手性** — 暂未复现，但（也许）能提供解释
- [ ] **分流器-汇流器紧贴时的优先级** — 部分解释（Splitter 的 `_build_distance_groups` 已将 Converger 提升为最高优先级组）
- [ ] **4n 卡 1 现象** — **无法解释**。实验结果显示在特定拓扑下，4 条输入线中只有 1 条能稳定输出，其余 3 条被阻塞，与预期的 round-robin 公平分配不符。目前仿真引擎无法复现这一行为。
- [ ] **部分实验复现结果不佳** — `expr_results/expr_1.txt` 记录了 24 种放置排列的实验数据，其中部分排列的仿真结果与原作游戏行为存在偏差（如"传送带优先"的判定在特定排列下与预期不一致）。具体偏差尚未定位到根因。
